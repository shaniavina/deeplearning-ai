...
# 
# **Note**: More epochs not always guarantee great results. Always remember to experiment! 
# 
# At the [bottom](#bottom) of the notebook you will find a training code which uses TensorFlow’s [enable_op_determinism](https://www.tensorflow.org/versions/r2.9/api_docs/python/tf/config/experimental/enable_op_determinism). “_When op determinism is enabled, TensorFlow ops will be deterministic. This means that if an op is run multiple times with the same inputs on the same hardware, it will have the exact same outputs each time._"
